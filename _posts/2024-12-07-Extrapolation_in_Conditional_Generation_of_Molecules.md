...
======
In this [fork](https://github.com/nofitsegal/e3_equiv_diff_mol_gen/tree/main), I've introduced additional features for conditional molecule generation. Users can use this to specify target property values for controlled molecule generation. Additionally, this implementation supports extrapolation, allowing users to explore molecules specifically generated to match property values outside the training range. The model gets at inference a range of desired property values, and generates 100 molecules along equal spacings of that range. To validate the property values of the generated molecules, I'm running quantum mechanics (QM) calculations using Density Functional Theory (DFT). These calculated values are accurate enough to be considered ground truth and can be used to evaluate the model's ability to generate stable and valid molecules with OOD property values. 
